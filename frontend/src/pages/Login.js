...
function LoginForm() {
  ...
  const handleGoogleSuccess = async (response) => {
    try {
      ...
      const placeholderPassword = "google-auth-placeholder-password";
      ...
      const signupResponse = await axios.post(`${process.env.REACT_APP_BASE_URL}/user/signup`, {
        ...
        userPhone: data.phone || "", // Phone is optional
      ...
      if (signupResponse.status === 201 || signupResponse.status === 400) {
        ...
        alert("An error occurred while creating or logging in the user.");
      }
    } catch (error) {
      console.error("Google login error:", error);
      if (error.response) {
        // If there's a response from the server, log the response data
        console.error("Server Response:", error.response.data);
      } else if (error.request) {
        // If there's no response but the request was made, log the request
...
//             </div>
//           </div>
//           {error && <div className="text-red-600 text-sm">{error}</div>}  {/* Error message */}
//           <div className="flex items-center">
//             <input
//               id="remember-me"
//               name="remember-me"
